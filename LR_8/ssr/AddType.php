...
require_once '../models/TypesList.php';
...
$IdC = '';
if(isset($_GET['id'])) {
    $data = $tl->getItemById($_GET['id']);
    $IdC = $data['id'];
    $typeC = $data['type'];
}
if(isset($_POST['type'])) {
    if($_POST['id']=='') {
        $tl->addToDB($conn, array('type'=>$_POST['type']));
    }
    else {
        $tl->updateDB($conn, array('id'=>$_POST['id'], 'type'=>$_POST['type']));
    }
    ...
    <body>
        <div class="container">
            ...
            <div class="mt-3">
                <form method = "POST">
                    <p>
                    <input class="form-input" type="text" value="<?php echo $typeC; ?>" name = "type"  placeholder="Type name" required/>
                    </p>
