...
require_once '../models/Coffee_machines.php';
...
class Coffee_machinesList extends BaseList {
    ...
    public function getBySearchQuery($conn, $query) {
        $stmt = $conn->prepare("SELECT m.machine_id, m.model, p.producer_name, t.type_name, c.coffee_name, pt.power_name 
                FROM coffee_machines m, producers p, types t, coffee_types c, power_types pt
                WHERE m.producer_id = p.producer_id 
                AND m.type_id = t.type_id 
                AND m.coffee_id = c.coffee_id 
                AND m.power_id = pt.power_id
                AND (m.model LIKE CONCAT('%',?,'%') OR p.producer_name LIKE CONCAT('%',?,'%') OR t.type_name LIKE CONCAT('%',?,'%') OR c.coffee_name LIKE CONCAT('%',?,'%') OR pt.power_name LIKE CONCAT('%',?,'%'))
                ORDER BY 1;");
        $stmt->bind_param("sssss", $query, $query, $query, $query, $query);
        $stmt->execute();
        $stmt->store_result();
        $stmt->bind_result($machine_id, $model, $producer_name, $type_name, $coffee_name, $power_name);
        while ($row = $stmt->fetch()) {
            $row = array(
                ...
                'type_id'        => $type_name,
                'coffee_type_id' => $coffee_name,
                'power_type_id'  => $power_name
            );
            $this->add($row);
        }
    }
