...
function displayCoffeeMachines() {
fetch('../api/Coffee_machines-json.php')
    .then((response) => response.json())
    .then((data) => {
        cmData = data;
        let content = '';
...
fetch('../api/Coffee_types-json.php')
    .then((response) => response.json())
    .then((data) => {
        ctData = data;
        let content = '';
...
fetch('../api/Power_types-json.php')
    .then((response) => response.json())
    .then((data) => {
        ptData = data;
        let content = '';
...
document.addEventListener('submit', function(e) {
    ...
    if(e.target.id =="cmf") {
        ...
        let fproducerid = document.querySelector('#cmf input[name="producer_id"]').value;
        let ftypeid = document.querySelector('#cmf input[name="type_id"]').value;
        let fcoffeetypeid = document.querySelector('#cmf input[name="coffee_type_id"]').value;
        let fpowertypeid = document.querySelector('#cmf input[name="power_type_id"]').value;

        if (fid == '') {
            let data = JSON.stringify({"model": fmmodel, "producer_id": fproducerid, "type_id": ftypeid, "coffee_type_id": fcoffeetypeid, "power_type_id": fpowertypeid});
            fetch("../api/Producers-json.php", {
                method: 'POST',
                headers: {
                    'Accept': 'application/json, text/plain, */*',
            ...
            .then(function(res) { return res.json(); })
            .then(function(data) { displayCoffeeMachines(); cmf.reset(); document.querySelector('#cmf input[name="item_id"]').value = ''; })
        }
        else {
...
document.addEventListener('click', function(e) {
    if(e.target.classList.contains('edit-coffeeMachine')) {
        ...
        for (i=0; i<cmData.length; i++) {
            if(cmData[i]['id'] == e.target.getAttribute('data-id')) {
                ...
                document.querySelector('#cmf input[name="producer_id"]').value = itemData['producer_id'];
                document.querySelector('#cmf input[name="type_id"]').value = itemData['type_id'];
                document.querySelector('#cmf input[name="coffee_type_id"]').value = itemData['coffee_type_id'];
                document.querySelector('#cmf input[name="power_type_id"]').value = itemData['power_type_id'];
            }
        }
    } 
        ...
        let data = JSON.stringify({"id":e.target.getAttribute('data-id')});
            ...
            .then(function(res) { return res.json(); })
            .then(function(data) { displayCoffeeMachines();})
    }
    ...
    if(e.target.id =="cmb") {
        ...
        e.preventDefault();
        fetch("../api/profile.php?action=logout")
            .then(function(res) { return res.json(); })
            .then(function(data) { 
                mainnav.style.display = "none";
                loginf.style.display = "block";
